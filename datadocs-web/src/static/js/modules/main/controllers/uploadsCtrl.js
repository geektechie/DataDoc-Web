define(['./module', 'angular', 'common', 'lodash', 'moment', 'KeyJS', 'notifications-utils'], function (controllers, angular, common, _, moment, KeyJS, NotificationsUtils) {
  ...
  controllers.controller('uploadsCtrl', ['$scope', '$rootScope', '$http', '$compile', '$uibModal', '$sce',
    ...
    'RecentSourcesService', 'DbSourceService','Account',
    function ($scope, $rootScope, $http, $compile, $uibModal, $sce, $q, $timeout, $filter, WSocket,
      ...
      BookmarkCommitService, UserEventService, ShareService, RecentSourcesService, DbSourceService,Account) {
      ...
      $scope.account.hasOutstandingPayment = false;
      $scope.account.hasOutstandingPayment = !$scope.account.isPaymentDone;
      ...
      function isMemberSelected(m){
        return _.some($scope.selectedMembers, (member) => member.id === m.id);
      }
      ...
      const setActiveSection = (section) => {
       ...
       $scope.clickedTeamMember = false;
      $scope.toggleActiveSection = function (section, suppressStateChange, ignoreFolder) {
        const newSection = SectionNamesService.getSectionNameFromSection(section);
        const userSection = SectionNamesService.getSectionNameFromSection(SECTIONS.USER_SETTINGS);
        ...
        if($scope.activeSection === SECTIONS.TEAM_MEMBERS){
            Account.getAccount();
            $scope.clickedTeamMember = true;
        }else{
            $scope.clickedTeamMember = false;
        }
        ...
        if(!suppressStateChange) {
            if (section === SECTIONS.MY_DATA) {
                $state.transitionTo(userSection, {}, { reload: false, inherit: true, notify: true });
                $timeout(function () {
                    $timeout(function () {
                        $timeout(function () {
                            $state.transitionTo(newSection, {f: $scope.selectedSourceFolder && !ignoreFolder ? $scope.selectedSourceFolder.id : null}, { reload: false, inherit: false, notify: true });
                        }, 10);
                    }, 10);
                }, 30);
            } else {
                ...
                $timeout(function () {
                    $timeout(function () {
                        $timeout(function () {
                            $state.transitionTo(newSection, {}, { reload: false, inherit: true, notify: true });
                        }, 10);
                    }, 10);
                }, 30);
            }
      ...
      $scope.showAddMemberModal = function() {
         $scope.addMemberModal = $uibModal.open({
           templateUrl: 'static/templates/include/add-member.html',
      ...
      $scope.lastSelectedMember = false;
      ...
      $scope.resetSelectedMember = function(){
          $scope.selectedMembers = [];
          $scope.lastSelectedMember = undefined;
          $scope.highlightedMember = {};
        };
      ...
      function getAllSortedMembers() {
        return $scope.account.members;
      }

    $scope.membersSingleClick = function(f, e){
        var isCtrlKey = e && !e.shiftKey && (e.ctrlKey || e.metaKey),
          isShiftKey = e && e.shiftKey;

        let [selectedItems, lastSelectedItem, highlightedMember] = [$scope.selectedMembers, $scope.lastSelectedMember, $scope.highlightedMember];
        ...
        if(isCtrlKey){
          if(e.which === KeyJS.UP || e.which === KeyJS.DOWN){
            ...
            if(!isMemberSelected(f)){
              selectedItems.push(f);
            } else {
              _.remove(selectedItems, function(m){ return m.id == f.id})
            }
          }
          lastSelectedItem = f;
        } else if(isShiftKey) {
          ...
          if(!lastSelectedItem){
            ...
            allSorted = getAllSortedMembers(f);
          }
        ...
        $scope.currentSelectedMember = f;
        $scope.lastSelectedMember = lastSelectedItem;
        $scope.selectedMembers = selectedItems;
        $scope.highlightedMember = highlightedMember;

        let members = {
          selected:  $scope.currentSelectedMember,
          allSelected: $scope.selectedMembers
        };
      ...
      $scope.mainPageMainMenuOptions = function(){
        if($scope.activeSection == SECTIONS.TEAM_MEMBERS){
            return [[ getMainPageMenuOptionsTemplate('Add Member', 'index-icon icon-small'), function() {
                                return $scope.showAddMemberModal();
                              }]];
        }
      ...
      $scope.memberContextMenuOptions = {
          dropdownMenuClass: 'main-page-dropdown-menu',
          dropdownItemClass: 'main-page-dropdown-menuitem',
          enabled: function(event){
            var o = $(event.originalEvent.target);
            ...
            var itemId = o.closest('.upload-item').find(".uid").val();
            var item = $scope.getMemberById(itemId, $scope.activeSection);
            return event.type == 'contextmenu';
          }
      };
      ...
      $scope.memberItemOptions = function(event) {
          var menu = [], itemName;
          var memberId = $(event.target).closest('.upload-item').find('.uid').val();
          var u = $scope.getMemberById(memberId, $scope.activeSection);
          if (!isMemberSelected(u)) {
             $scope.membersSingleClick(u, event);
          }
          let items = $scope.selectedMembers;

          let deleteOption = ["<i class='fa fa-trash icon'></i>Delete", function(){
                                           event.stopPropagation();
                                           return $scope.showRemoveMemberPopup();
                                         }];

          let addMemberOption = ["<i class='fa fa-user icon'></i>Add Member", function(){
                                                  event.stopPropagation();
                                                  return $scope.showAddMemberModal();
                                                }];
          if(items.length > 1)
          ...
          {
            if(items.length == 1 && User.getCurrent().email === items[0].email)
            {
                menu.push(addMemberOption);
            }
            else
            {
                menu.push(addMemberOption);
      ...
      $scope.getMemberById = function(mid, section = $scope.activeSection){
          let member;
          let members = $scope.account.members;
          _.each(members, function (m) {
            if (m.id === Number(mid)) {
              ...
              if (m.sections) {
                let section = _.find(m.sections, s => s.id === mid);
                if (section) {
                  member = section;
                  return false;
                }
      ...
      function updateServerTimeDiff(res) {
        ...
        var clientTime = new Date(Date.now()).getTime();NAccount
      ...
      $scope.sortTeamMembers = function(options = {}){
        let order = options.sort.desc?'asc':'desc';
        var sortedUsers = _.sortByOrder($scope.account.members, [options.sort.field], [order]);
        $scope.account.members.splice(0, $scope.account.members.length);
        sortedUsers.forEach(e => {
            $scope.account.members.push(e);
        })
      };
      ...
      $scope.showRemoveMemberPopup = function(email, event) {
         $scope.memberToBeDeleted = email;
         $scope.removeMember = $uibModal.open({
           templateUrl: 'static/templates/include/remove-member.html',
           scope: $scope,
           animation: true,
           size: 'md'
         });
      ...
      $scope.cancelRemoveMemberModal = function(){
        $scope.removeMember.dismiss();
      };

      $scope.removeMembers = function() {
        if($scope.memberToBeDeleted){
            Account.removeMembers([$scope.memberToBeDeleted]).then(function(response) {
                 $scope.memberToBeDeleted = undefined;
                 $scope.removeMember.dismiss();
            }).catch(err => {
                console.log('Error while deleting member', err);
            });
        }else{
            let currentUser = User.getCurrent();
            var emailIds = _.map($scope.selectedMembers, function(m) {return m.email});
            emailIds = _.filter( emailIds, function(id) { return id != currentUser.id} );
            Account.removeMembers(emailIds).then(function(response) {
                $scope.memberToBeDeleted = undefined;
                $scope.removeMember.dismiss();
            }).catch(err => {
              console.log('Error while removing members by id');
            });
        }
      };
      ...
      $scope.modifyMember = function(role, email, event){
         Account.modifyMember(email, role).then(function(response) {
            if($scope.currentUser.email === email && role == 'Member'){
               User.updateUserRole(role);
               $scope.isCompanyAdmin = User.isCompanyAdmin();
               $scope.activeSection = SECTIONS.MY_DATA;
               $state.go('main.landing.my_data');
            }
         }).catch(err => {
             console.log('Error while modify member.');
           });
      };

      $scope.columnsSectionsTeamMembers = {
        sources: SortSectionColumnsService.genTeamMembersSortColumnsOptions($scope)
      };

      $scope.memberIsLoggedInUser = function(m){
        return User.getCurrent().email === m.email;
      };
      ...
      $scope.ngSelectMembersSectionOptions = {
          itemSelector: '.upload-item',
          itemSelectorWrapper: '.main-page-ds-pane',
          getItemFn: $scope.getMemberById,
          selectedItemsKey: 'selectedMembers',
          lastSelectItem:'lastSelectedMember',
          currentSelectedItem:'currentSelectedMember',
          onClick: $scope.membersSingleClick,
          keyboardNavOptions: {
            onSelectAll: function(e) {
              $scope.$evalAsync(selectAllMembers);
            },
            onEsc: function(e) {
              $scope.$evalAsync($scope.resetSelectedMember);
            },
            enableAutoSelect: true
          }
        };
      ...
      function selectAllMembers() {
        ...
        $scope.selectedMembers.push(...$scope.account.members);
      }
