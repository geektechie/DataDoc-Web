define([
    ...
    'TimeZoneService', '$location', 'RecentSourcesService', 'User', 'Account', 'ShareService', 'RouterService', 'FilterService',
    ...
    function ($rootScope, $scope, $http, $localStorage, $timeout, $stateParams, $q, $state,
              ...
              FormatToolbarService, ColumnsService, cronService, TimeZoneService, $location, RecentSourcesService, User, Account,
        ...
        function displayNotification(totalRow){
            if(User.getCurrent().account && User.getCurrent().account.planType === 'FREE' && totalRow > Account.getUserFreeLimit()){
                var datadocId = $scope.datadocId;
                if($localStorage[datadocId] == null){
                    const message = 'This datadoc has been limited to ' + Account.getUserFreeLimit() + ' rows. To view all rows, you will need to upgrade to a <span type="button" id="upgradeAccount"><a> paid account</a></span>.';
                    NotificationsUtils.notify({ message }, { type: NotificationsUtils.NotificationType.SUCCESS, delay: 5});
                    $localStorage[datadocId] = true;
                }
        ...
        $scope.deserializeIngestSettingsFromJSON = function(json) {
            if (json) {
                ...
                return $http.post('/api/files/get_file', {path: 'id:' + selectedSourceId, sections: true, relatedDatadocs: true})
                    .then(response => {
                        displayNotification(response.data.descriptor.rowsCount)
