define(['./module', 'common', 'lodash', 'angular'], function(module, cc, _, angular) {
    module.service('SortSectionColumnsService',
        ...
        function getTeamMembersColumnOptions(teamMemberParams, parentObj) {
            var teamMemberColumn = {
                class: teamMemberParams.class || 'col-md-3 col-sm-3 col-xs-3',
                innerClass: teamMemberParams.innerClass || 'index-header',
            };
            teamMemberColumn.choices = teamMemberParams.choices;
            teamMemberColumn.selected = teamMemberParams.selected;
            teamMemberColumn.sort = teamMemberParams.sort;

            if (teamMemberParams.withHeaderClickHandler) {
                if (teamMemberParams.onHeaderClick) {
                    teamMemberColumn.onHeaderClick = teamMemberParams.onHeaderClick;
                } else {
                    teamMemberColumn.onHeaderClick = function() {
                        parentObj.onSort(null, this);
                    }
                }
            }
            return teamMemberColumn;
        }
        ...
        function genTeamMembersSortColumnsOptions($scope,search) {
            const listOptions = _.merge({},
                    getSectionOptions('.sources_section_columns.SourcesSectionColumnsSelectedChangeEvent',
                    sort => $scope.sortTeamMembers({sort}), $scope)
                );
            const firstColumn = getTeamMembersColumnOptions({
                selected:{
                    title: 'Email',
                    sortProperty: 'email'
                },
            ...
            const secondColumn = getTeamMembersColumnOptions({
                selected: {
                      title: 'Role',
                      sortProperty: 'role'
                },
                ...
                innerClass: 'index-header',
                withHeaderClickHandler: true
            }, listOptions);

            const thirdColumn = getTeamMembersColumnOptions({
                selected:{
                      title: 'Date Added',
        ...
        return {
            ...
            genTeamMembersSortColumnsOptions: genTeamMembersSortColumnsOptions,
