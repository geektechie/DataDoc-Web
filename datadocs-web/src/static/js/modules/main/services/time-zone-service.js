define(['./module'], function (module) {
    module.service('TimeZoneService', [function () {
        ...
        return {
            ...
            getDefaultTimezone: () => timeZones[3],
            findByName: (name) => timeZones.find( tz => tz.name === name),
            findByAbbr: (abbr) => timeZones.find( tz => tz.abbr === abbr),
            findByText: (text) => timeZones.find( tz => tz.text === text)
        }
    }]);
});