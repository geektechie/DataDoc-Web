define(['./module', 'angular', 'common', 'lodash', 'moment', 'pluralize', 'ag-grid', 'fin-grid', 'notifications-utils', 'jquery'],
    function (services, angular, cc, _, moment, pluralize, agGrid, finGrid, NotificationsUtils, jquery) {
        services.service('DataLoadingService', ['$q', '$http', '$timeout', '$rootScope', '$filter', '$compile', 'WSocket',
            ...
            'BookmarkEventService', 'FormatCellService', 'User', '$uibModal',
            function ($q, $http, $timeout, $rootScope, $filter, $compile , WSocket, ScopeService, SearchBarService,
                      AvailableFixedDates, EventNames, Constants, ColumnsService, BookmarkEventService, FormatCellService, User, $uibModal) {
                ...
                function initGrid(params, $scope) {
                    ...
                    if (!$scope.gridOptions || !$scope.gridOptions.isAlive || !$scope.gridOptions.isAlive()) {
                       ...
                       window.grid.behavior.featureMap.ColumnMoving.originalMovePlaceholderTo = window.grid.behavior.featureMap.ColumnMoving.movePlaceholderTo;
                    }
                }
                                ...
                                getRows: function (params) {
                                    ...
                                    var cb = function (data) {
                                        if (params.startRow == 0) {
                                            ...
                                            var colDefs = createColumnDefs($scope);
                                            $scope.gridOptions.api.setColumnDefs(colDefs);

                                            window.grid.behavior.featureMap.ColumnMoving.movePlaceholderTo = function(grid, column){
                                                console.log('Called overridden movePlaceholderTo...');
                                                this.originalMovePlaceholderTo(grid, Math.min(column,colDefs.length));
                                             }
                                        }
                                        var lastRow = -1;
                ...
                function restore(tab, gridId, $scope, options){
                    ...
                    if(state.notifications){
                        _.each(state.notifications, (e) => {
                            ...
                            var isFree = User.getCurrent().account.planType === 'FREE';
                            if(isFree && e.limited){
                                e.message = e.message + ' <span type="button" id="upgradeAccount"><a> paid account</a></span>.';
                            }
                    ...
                    $('#upgradeAccount').on('click', 'a', function(e) {
